 ...
 *       请求的协议的版本号 HTTP/1.1
 *    (2)请求头
 *       key : value 组成 不同的键值对，表示不同的含义（比如，请求发送的服务端域名是什么，希望接收的响应消息使用什么语言，请求消息体的长度等等
 *  post请求
 *  （1）请求行
 *       请求的方式 POST
 *       请求的资源路径[+?+请求参数]
 ...
 *  5xx 表示服务端在处理请求中，发生了未知的错误。通常是服务器内部错误（代码错误）
 *
 * 【MIME】了解即可
 *  MIME是HTTP协议中数据类型。有时候我们需要准确的告知浏览器我们传入的数据类型，防止浏览器曲解
 *  MIME 类型的格式是“大类型/小类型”
 *  常见MIME类型：
 *         文件                                              MIME类型
