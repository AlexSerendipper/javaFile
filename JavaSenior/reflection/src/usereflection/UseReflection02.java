 ...
 *  ✔✔加载到内存中的类，我们称为运行时类，此运行时类，就作为Class类的一个实例。。。即一个类本身也是一个对象
 *   换句话说，Class类的实例，就对应着一个运行时类
 *  注意：加载到内存中的运行时类，会缓存一定的时间
 *   在此时间内我们可以通过不同的方式，获取的都是同一个运行时类，他们地址值相同
 * 2）链接link
 *  将类的二进制数据合并到JRE中
 * 3)初始化initialize
 *  执行类构造器<clinit>()方法的过程
 ...
 *
 *【类加载器classloader】把类(class)装载进内存的,相当于类的加载过程中的运行阶段
