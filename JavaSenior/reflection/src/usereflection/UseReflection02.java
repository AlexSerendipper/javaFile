 ...
 *    虚拟机自带的加载器，一般不涉及
-----------------------
 为保证安全性，实际上类加载器采用了双亲委派机制✔✔✔✔✔实际上类加载器是层层递进的一层层往下寻找（根加载器==>扩展类加载器==>系统类加载器）
   解释：当一个类加载器收到类加载请求，它首先不会自己去尝试加载这个类，而且把请求委托给父类加载器去加载，直到所有的加载请求最终到达顶层的启动类加载器
         只有当父类加载器反馈自己无法完成加载请求时（指它的搜索范围没有找到所需的类），子类加载器才会尝试自己去加载。如果都无法加载则抛出异常（classNotFound）
 双亲委派机制的作用：
   1、防止重复加载同一个.class。通过委托去向上面问一问，加载过了，就不用再加载一遍。
   2、保证java核心库的安全性。通过委托方式，不能去篡改核心类文件（即使篡改也不会加载）。这样保证了Class执行安全。
