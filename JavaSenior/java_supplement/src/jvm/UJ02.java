 ...
 *  存放内容：
 *   ✔✔✔ 静态变量(static)、运行时常量池（直接引用、数值型字面量）、类信息(.Class运行时类)。
 *   注意：1）实例变量存在堆内存中，和方法区无关
 *         2）jdk1.7之后，静态变量改为存储在堆中
 *
 * 【栈】或称为虚拟机栈
 ...
 * （1）classLoader将 class文件字节码 内容加载到内存中，即 将这些静态数据转换成方法区的运行时数据结构，并生成一个代表这个类的java.lang.Class对象（运行时类对象）
 *                                                   以及 将这些静态数据转换成堆空间的实例数据（对象具体的实例）,并将这些实例与栈建立对象引用
 *                                                   并且把堆中的 实例链接到方法区的运行时数据结构上
 * （2）栈空间只存放 堆空间的实例对象的引用
 * （3）堆空间中的常量，引用自 方法区中的常量池（运行时常量池）
 *
 @author Alex
