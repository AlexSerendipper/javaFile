 ...
 *                      redis默认使用watch指令进行监视 乐观监控, 如果在事务执行之前 被监控的key被其他命令改动，导致版本号就不一致，当前事务就会被打断
 ...
 *    （提交前检查指令的语法才会提交，如果一旦提交了，将全部执行）
 *     (执行阶段某个命令出错，只有出错命令不被执行，其余都会被执行完)
 *     (并且Redis不支持直接回滚，但我们可以通过 Redis提供的watch命令来实现回滚)
