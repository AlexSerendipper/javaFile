 ...
 * 2. 格式：①->: 称为lambda操作符 或 箭头操作符
 *         ②箭头左侧为原抽象方法的形参列表
 *         ③箭头右侧为重写的抽象方法体
 * 3. 使用（分六种情况）：见下
 *    总结：①箭头左侧，lambda表达式形参列表的参数类型可以省略（类型推断），如果只有一个参数，小括号也可以省略
 *          ②箭头右侧，lambda体应该用一对大括号包裹，如果lambda中只有一条执行语句，可以省略return和大括号
 * 4. ✔本质：快速创造了一个实现接口的实例（要求此接口只能有一个抽象方法，即函数式接口）
 * 5. 使用场景：当需要对一个函数式接口进行实例化时，可以使用函数式接口
 *             所以以前用匿名实现类表示的现在都可以用Lambda表达式来写。
 *
 * 【函数式接口】
 * (1)只包含一个抽象方法的接口，称为函数式接口。
 * (2)可以在一个接口上使用 @FunctionalInterface注解，检查它是否是一个函数式接口(编译器检查)
 * (3)Java内置四大核心函数式接口。其他接口见xmind图
 ...
 * (4)何时使用函数式接口：如果开发中需要定义一个函数式接口，首先看看jdk已提供的函数式接口是否能满足需求，如果能，则直接调用即可
 *
 @author Alex
