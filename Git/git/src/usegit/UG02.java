 ...
 *                                       如果我们将文件保存到暂存区后又对文件进行修改这时查看git status
 *                                       ✔（1）若只是修改内容，可以再次add，覆盖暂存区的同名文件（2）若是将文件改名，则改名前的文件仍会存在在暂存区中，需要手动删除。改名后的文件作为新文件需要被首次跟踪
 *
 ...
 *                                     pull后，会将版本同步至与远程库相同，若此时对本地库做了修改（不push到远程库），是无法再次pull的（因为版本号总是相同，会显示当前为最新版本），此时可以选择先回退版本后再进行pull。
 *
 * 【6、删除操作】我们通常不会去动本地库的内容
 *  git reset HEAD	当前目录下文件名             # 在暂存区移除指定文件，重新放回工作区
 *  git reset .                               # 移除暂存区所有文件，重新放回工作区
 *  git rm	当前目录下文件名                     # 与直接从目录手动删除文件效果一致✔。当在当前版本下，删除文件时有两种情况，
 *                                                （1）一是确实要从版本库中删除该文件，那就用命令git rm删掉，并且git commit。此时文件就从版本库（本地库）中被删除了。
 *                                                （2）另一种情况是删错了，因为版本库里还有呢，所以可以使用checkout命令把误删的文件恢复到最新版本：
 *  git checkout -- 文件名                    # 用本地仓库里的指定内容 覆盖 工作区的指定内容，这里有两种情况：
 *                                                （1）一种是文件修改后还没有被放到暂存区，该命令可以直接把本地仓库里的内容覆盖掉工作区的内容；
 *                                                （2）一种是文件已经添加到暂存区后，此时需要 git reset HEAD	文件名，将暂存区的文件放回工作区，然后再执行checkout命令
 *                                                    若直接执行checkout命令，会将工作区的内容覆盖，但暂存区不变
 *  git rm --cached 当前目录下文件名           # 从工作区和暂存区中删除文件（保留本地文件），✔并取消该文件的版本控制
 *
 *
 @author Alex
