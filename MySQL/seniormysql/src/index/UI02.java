 ...
 *  概念：聚簇索引并不是一种单独的索引类型，而是一种数据存储方式（所有的用户记录都存储在了叶子节点），
 *         聚簇索引中，每个索引指向的即为数据值，也就是所谓的索引即数据，数据即索引。✔上述推导的B+树即为聚簇索引✔
 *         MySQL中，InnoDB存储引擎会自动的为我们创建聚簇索引✔。
 *        （正如innodb引擎中，使用的idb文件，索引即数据。而MyISAM引擎中，索引和数据分开存储）因此innodb是聚簇索引，myisam是非聚簇索引
 *  特点（b+树的特点）：
 ...
 *  优点：
 *      数据访问更快，因为聚簇索引将索引和数据保存在同一个B+树中，因此从聚簇索引中获取数据比非聚簇所以更快（无需回表操作）
 *      排序查找和范围查找速度非常快：按照聚簇索引排列顺序，查找显示一定范围数据的时候，由于数据都是紧密相连，数据库不用从多个数据块中提取数据，所以节省了大量的IO操作
 *  缺点：
 ...
 *  特点：
 *      页的记录是按照（见xmind） c2列的大小顺序 排成一个单向链表。
 *      各个存放用户记录的页（第0层）也是根据页中记录的 c2列大小顺序 排成一个双向链表。
 *      存放目录项记录的页（除第0层外的所有层）分为不同的层次，在同一层次中的页也是根据页中目录项记录的c2列大小顺序排成一个双向链表。
 *      ✔B+树的叶子节点存储的并不是完整的用户记录，而只是c2列 和 主键（c1列）的值（数据位置）✔✔✔
 *          即目录项记录页 中不再是主键+页号的搭配，而变成了c2列+页号的搭配✔✔✔
 *  回表：
