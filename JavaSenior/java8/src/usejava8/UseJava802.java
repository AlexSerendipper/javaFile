...
/**
 * 【方法引用】
 *  方法引用，本质上就是lambda表达式的更简化形式。故其本质也是作为函数式接口的实例
 *  使用格式：类（或对象）：：调用方法名
 *  使用情景：✔✔✔只能用于lambda表达式中只有一条执行语句(方法)，即可以省略return和大括号的情况。。 具体分为如下的三种情况
 *
 *    对象::非静态方法（实例方法）✔      # 要求接口（函数式接口）中的抽象方法的形参列表和返回值类类型 与 方法引用的方法的形参列表和返回值类型 完全相同
 *    类::静态方法 ✔                   # 要求接口（函数式接口）中的抽象方法它的形参列表和返回值类型 与 方法引用的方法的形参列表和返回值类型 完全相同
 *    类::非静态方法（实例方法）✔        # 要求接口中抽象方法的形参，作为非静态方法的调用者出现
 *
 * 【构造器引用】
 *  使用条件：接口中的抽象方法的形参列表与构造器形参列表相同，其返回值类型就是构造器所属的类的类型
 *  使用格式为: 类名::new
 *
 * 【数组引用】
 *  使用条件：接口中的抽象方法的形参列表刚好作为数组的长度，返回值类型就是数组的类型
 *  使用格式： type[]::new
 *
 @author Alex
