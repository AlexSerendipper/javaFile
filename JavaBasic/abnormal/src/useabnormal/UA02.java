 ...
 * 【抓】抓
 *  Java提供的是异常处理的抓抛模型
 *  Java程序的执行过程中如出现异常，会生成一个异常类对象（抓），该异常对象将被提交给Java运行时系统，这个过程称为抛出(throw)异常。
 *  一旦异常对象抛出后，其后的代码不再执行
 *
 * 【抛】
 *  异常对象的生成
 *    由虚拟机自动生成：程序运行过程中，虚拟机检测到程序发生了问题，如果在当前代码中没有找到相应的处理程序，
 *   就会在后台自动创建一个对应异常类的实例对象并抛出——自动抛出✔
 *    由开发人员手动创建：Exception exception = new ClassCastException();——手动抛出✔
