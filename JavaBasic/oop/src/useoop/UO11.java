 ...
 *  ✔在static方法内部只能访问类的static修饰的属性或方法，不能访问类的非static的结构（省略了类名.属性）
 *      非静态方法既可以调用静态属性方法，也可以调用非静态属性方法（省略了this.属性）
 *  static方法内部不能有this和super。 因为静态方法存在时，当前实例还未存在
 *  static修饰的方法不能被重写
 *  在开发中，1）不同对象共享的属性 2)常量，通常都设置为静态属性；
 *            1）操作静态属性的方法 2）工具类的方法，通常都设置为静态方法
 *
 * 【单例设计模式】
