 ...
 * 2）ArrayList是实现了基于动态数组(自动扩展大小)的数据结构，LinkedList基于链表的数据结构。对于
 * 随机访问get和set，ArrayList效率优于LinkedList，因为LinkedList要移动指针。对于新增
 * 和删除操作add(特指插入)和remove，LinkedList比较占优势，因为增加和删除操作ArrayList要整体移动数据。
 *  异（ArrayList和Vector）：
 * 1）Vector和ArrayList几乎是完全相同的,唯一的区别在于Vector是同步类(synchronized)，属于
 ...
 * 2. 查看add操作，首次添加时，last为null，造的第一个元素为（null,item1,null）,并将第一个元素作为last和l，以及first
